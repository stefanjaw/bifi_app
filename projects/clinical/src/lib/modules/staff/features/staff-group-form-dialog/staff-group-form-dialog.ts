...
import { StaffGroupForm } from '../../services/staff-group-form';
import { staffGroup } from '../../interfaces/staff';

@Component({
  selector: 'bifi-app-staff-group-form-dialog',
  imports: [
    ReactiveFormsModule,
    DialogModule,
    SelectModule,
  ...
  templateUrl: './staff-group-form-dialog.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
/** Dialog for creating or editing a staff group */
export class StaffGroupFormDialog extends BaseDialog {
  private crud = inject(CrudStaffGroups);
  protected formService = inject(StaffGroupForm);
  private destroy$ = inject(DestroyRef);

  form = this.formService.form;
  editEntity = signal<staffGroup | null>(null);
  submitLoading = signal(false);
  isUpdate = signal(false);
  saved = output<void>();

  open(entity?: staffGroup): void {
