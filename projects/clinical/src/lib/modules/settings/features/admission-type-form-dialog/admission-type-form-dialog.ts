...
import { AdmissionTypeForm } from '../../services/admission-type-form';
import { admissionType } from '../../interfaces/settings';

@Component({
  selector: 'bifi-app-admission-type-form-dialog',
  imports: [
    ReactiveFormsModule,
    DialogModule,
    InputText,
  ...
  templateUrl: './admission-type-form-dialog.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
/** Dialog for creating or editing an admission type */
export class AdmissionTypeFormDialog extends BaseDialog {
  private crud = inject(CrudAdmissionTypes);
  protected formService = inject(AdmissionTypeForm);
  private destroy$ = inject(DestroyRef);

  form = this.formService.form;
  editEntity = signal<admissionType | null>(null);
  submitLoading = signal(false);
  isUpdate = signal(false);
  saved = output<void>();

  open(entity?: admissionType): void {
