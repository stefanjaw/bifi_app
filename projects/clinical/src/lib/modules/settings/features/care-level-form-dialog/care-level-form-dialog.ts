...
import { CareContinuumLevelForm } from '../../services/care-level-form';
import { careContinuumLevel } from '../../interfaces/settings';

@Component({
  selector: 'bifi-app-care-level-form-dialog',
  imports: [
    ReactiveFormsModule,
    DialogModule,
    InputText,
  ...
  templateUrl: './care-level-form-dialog.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
/** Dialog for creating or editing a care continuum level */
export class CareLevelFormDialog extends BaseDialog {
  private crud = inject(CrudCareContinuumLevels);
  protected formService = inject(CareContinuumLevelForm);
  private destroy$ = inject(DestroyRef);

  form = this.formService.form;
  editEntity = signal<careContinuumLevel | null>(null);
  submitLoading = signal(false);
  isUpdate = signal(false);
  saved = output<void>();

  open(entity?: careContinuumLevel): void {
