...
import { MaritalStatusForm } from '../../services/marital-status-form';
import { maritalStatus } from '../../interfaces/settings';

@Component({
  selector: 'bifi-app-marital-status-form-dialog',
  imports: [
    ReactiveFormsModule,
    DialogModule,
    InputText,
  ...
  templateUrl: './marital-status-form-dialog.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
/** Dialog for creating or editing a marital status */
export class MaritalStatusFormDialog extends BaseDialog {
  private crud = inject(CrudMaritalStatuses);
  protected formService = inject(MaritalStatusForm);
  private destroy$ = inject(DestroyRef);

  form = this.formService.form;
  editEntity = signal<maritalStatus | null>(null);
  submitLoading = signal(false);
  isUpdate = signal(false);
  saved = output<void>();

  open(entity?: maritalStatus): void {
