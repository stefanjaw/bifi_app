...
import { FluidTrackForm } from '../../services/fluid-track-form';
import { fluidTrack } from '../../interfaces/fluid-tracks';

@Component({
  selector: 'bifi-app-fluid-track-form-dialog',
  imports: [
    ReactiveFormsModule,
    DialogModule,
    InputText,
  ...
  templateUrl: './fluid-track-form-dialog.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class FluidTrackFormDialog extends BaseDialog {
  private crud = inject(CrudFluidTracks);
  protected formService = inject(FluidTrackForm);
  private destroy$ = inject(DestroyRef);

  form = this.formService.form;
  editEntity = signal<fluidTrack | null>(null);
  submitLoading = signal(false);
  isUpdate = signal(false);
  saved = output<void>();

  open(entity?: fluidTrack): void {
