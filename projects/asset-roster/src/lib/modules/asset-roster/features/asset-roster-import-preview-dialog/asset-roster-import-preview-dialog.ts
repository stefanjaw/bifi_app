...
/**
 * Preview dialog for the Asset Roster CSV import: lets the user pick which CSV
 * columns to import, previews the first records, validates the selection with
 * the backend, and finally runs the import.
 */
@Component({
  selector: 'bifi-app-asset-roster-import-preview-dialog',
  imports: [DialogModule, ButtonModule, TranslatePipe],
  templateUrl: './asset-roster-import-preview-dialog.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class AssetRosterImportPreviewDialog extends BaseDialog {
  ...
  toggleColumn(column: string, event: Event): void {
    ...
    this.selectedColumns.update(columns =>
      checked ? [...columns, column] : columns.filter(selectedColumn => selectedColumn !== column)
    );
  ...
  private async readCsvFile(file: File): Promise<void> {
    try {
      ...
      // The optional id column is importable.
      // MongoDB metadata fields remain excluded from the import.
  ...
  private createSelectedColumnsCsv(): File | null {
    ...
    return new File([csvContent], this.fileName() || 'asset-roster-import.csv', {
      type: 'text/csv',
    });
  }
  ...
  private getValidationErrorMessage(error: unknown): string {
    ...
    // Supports the raw backend response:
    // { message: string, errors: [{ path, messages }] }
    // It also supports the older `details` property if another endpoint uses it.
    ...
    // The global HTTP error interceptor converts backend errors to:
