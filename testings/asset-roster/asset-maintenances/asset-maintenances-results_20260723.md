...
Tested: 2026-07-21 (re-tested 2026-07-22 after fixes, re-tested 2026-07-23 with in-app navigation)
...
| 1.3 | Verify the section is gated by commission status | On an `awaiting-commissioning` asset, the service/PM buttons are disabled with a message | ✅ PASS — Initiate Service now always visible but disabled via `[disabled]="!canStartService()"` (AM-05 fix). PM disabled with alert message. |
| 1.4 | Verify the section is gated on a decommissioned asset | On a decommissioned asset, the service/PM buttons are disabled | ✅ PASS — Initiate Service always visible but disabled (AM-05 fix). PM disabled ("This asset is decommissioned."). |
...
| 6.3 | Verify the confirmation line | Shows "Confirm completion of" + the service name + "initiated on" + the start date | ✅ PASS — Double-colon typo fixed (AM-04: removed extra `:` from template). Shows "initiated on: Jul 21, 2026" |
...
| 6.9 | Submit with all fields empty (no notes, no cost, no attachments) | Validation error shown on Notes field; form does not submit | ✅ PASS — Notes field now has `NonWhitespaceValidators.nonWhitespaceRequired`. Error handler shows toast on API failure. B-01 resolved. |
...
| 10.2 | Verify the asset status changes | Status changes from `active` to `in-pm` | ✅ PASS — Maintenance page alert now shows "In PM" via separate `in-pm` branch (AM-03 fix). List correctly shows "In PM". |
| 10.3 | Verify the Maintenance Service section updates | "Initiate PM" is replaced; "Finish PM" and "Skip PM" buttons appear | ✅ PASS — Skip PM now always rendered (AM-02 fix). Initiate PM hidden when in-progress; Finish PM and Skip PM both visible. |
...
| 11.3 | Verify the confirmation line | Shows "Confirm completion of" + the PM name + "initiated on" + the start date | ✅ PASS — Double-colon typo fixed (AM-04: removed extra `:` from template). Shows "initiated on: Jul 21, 2026" |
...
| 11.9 | Submit with all fields empty | Validation error shown on Notes field; form does not submit | ✅ PASS — Notes field now has `NonWhitespaceValidators.nonWhitespaceRequired`. B-01 resolved. |
...
|---|------|-------------|----------|
| B-01 | ~ | **CRITICAL — Finish Service/PM with empty Notes (AM-01)** — ✅ **RESOLVED 2026-07-22**: Added `NonWhitespaceValidators.nonWhitespaceRequired` to notes. Error handler shows toast on API failure. Files: `update-maintenance-form.ts:18`, `asset-finish-maintenance-form-dialog.ts:99-101`. | **Fixed** |
| B-02 | ~ | **"Skip PM" button NOT shown when PM is in progress (AM-02)** — ✅ **RESOLVED 2026-07-22**: Moved Skip PM outside `@if (!pmStarted())` block. Now always rendered (disabled when not applicable). File: `maintenance-service-section.html:125-151`. | **Fixed** |
| B-03 | ~ | **Status alert shows "Active" instead of "In PM" (AM-03)** — ✅ **RESOLVED 2026-07-22**: Added separate `in-pm` branch with `status.inPm` label. File: `status-banner-section.html:15-23`. | **Fixed** |
| B-04 | ~ | **Double-colon typo (AM-04)** — ✅ **RESOLVED 2026-07-22**: Removed extra `:` after translation. File: `asset-finish-maintenance-form-dialog.html:18`. | **Fixed** |
| B-05 | ~ | **Initiate Service button HIDDEN (AM-05)** — ✅ **RESOLVED 2026-07-22**: Changed to always rendered with `[disabled]="!canStartService()"`. File: `maintenance-service-section.html:163-170`. | **Fixed** |
| B-06 | 3.5, 10.4, 19.1, 19.3 | **Activity history does NOT show entries immediately after initiating a service or PM** — Entries only appear after maintenance is finished/skipped. | Low |
| B-07 | ~ | **PM schedule fields remain locked after finishing PM (AM-07)** — ✅ **RESOLVED 2026-07-22**: Changed `isMaintenanceWindowsEditLocked` to check `pmStarted()` not `maintenanceWindowIds?.length`. File: `maintenance-service-section.ts:47-53`. | **Fixed** |
| B-08 | ~ | **Double toast on service creation (AM-08)** — ✅ **RESOLVED 2026-07-22**: Added `notificationConfig: { enable: false }` to service POST. File: `asset-maintenance-form-dialog.ts:72`. | **Fixed** |
...
> **Re-tested 2026-07-22 after fixes:** AM-05 (Initiate Service always visible) verified: button visible but disabled on decommissioned asset. AM-01 through AM-08 code changes verified by build. B-01 through B-05, B-07, B-08 moved to Fixed. 7 more PASS, 5 fewer BUG/NOTE/FAIL. Tests 1.3, 1.4, 6.3, 6.9, 10.2, 10.3, 11.3, 11.9 now PASS.
