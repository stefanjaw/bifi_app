...
| 4.8 | Fill Name with whitespace only, click Save | Validation error or document actual behaviour | ⚠️ BUG — Whitespace-only Name passes client-side validation; server creates record with empty name; appears in list without visible name |
...
|---|------|-------------|----------|
| B-01 | 4.8 | **Whitespace-only name accepted** — Filling Name with whitespace passes client-side validation; server creates record with empty/whitespace name. Record appears in list with no visible name. | Medium |
