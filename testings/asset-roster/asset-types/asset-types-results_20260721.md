...
| 4.6 | Fill Name with whitespace only, click Save | Whitespace name passes client-side validation; server trims whitespace and saves successfully | ⚠️ NOTE — Whitespace-only Name passes client validation; server trims to empty; record saved with empty name |
...
| 7.3 | Delete an asset type that is referenced by an asset roster | System allows deletion — "Type" record was deleted without reference error | ⚠️ BUG — Referenced asset type ("Type") deleted successfully with no warning; may leave dangling references in Asset Roster records |
...
| 10.3 | Enter a duplicate Name (same as existing "Other") | System allows it — two entries with "Other" name coexist | ✅ PASS — Duplicate name "Other" created with different description; no validation error |
...
| # | Test | Expected Result | Pass/Fail |
|---|------|----------------|-----------|
| 12.1 | Navigate to Asset Roster list and check "Type" column | Column shows asset type name or "Not set" fallback | ⏭️ N/A — Requires navigating to Asset Roster module |
| 12.2 | Open Asset Roster create/edit form and check asset type dropdown | Dropdown lists all asset types with "Other" option | ⏭️ N/A — Requires navigating to Asset Roster module |
| 12.3 | Select "Other" in the asset type dropdown | Additional fields appear for inline creation | ⏭️ N/A — Requires navigating to Asset Roster module |
| 12.4 | Fill inline fields and save asset roster | New asset type created and linked | ⏭️ N/A — Requires navigating to Asset Roster module |
| 12.5 | Filter Asset Roster list by asset type name | Matching assets appear | ⏭️ N/A — Requires navigating to Asset Roster module |
...
|---|------|-------------|----------|
| B-01 | 4.4 | **Save button only appears when form is dirty** — On both Create and Edit forms, the Save button is hidden until at least one field is modified. Users may not realize they need to type something before the Save button appears. Same pattern as contacts B-01. | Medium |
...
| B-05 | 4.6 | **Empty name accepted via whitespace** — Whitespace-only Name passes client-side validation and is accepted by the server (trimmed to empty). Records can be created with empty/null name. | Low |
