...
Tested: 2026-07-21 (re-tested 2026-07-22 after fixes, re-tested 2026-07-22 after AT-02/AT-04 fixes, re-tested 2026-07-23 with in-app navigation, re-tested 2026-07-24 after AT-03 fix)
Method: Automated UI tests via Playwright browser

> **2026-07-24 re-test scope:** AT-03 (orphaned references on asset type deletion) — backend `asset-type-service.ts` `delete()` override added. Pending test: requires backend restart and UI verification that deleting an asset type referenced by assets removes the type ID from those assets' `assetTypeIds` arrays.
