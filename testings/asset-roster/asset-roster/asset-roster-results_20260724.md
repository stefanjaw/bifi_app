...
Tested: 2026-07-21 (re-tested 2026-07-22, re-tested 2026-07-23 after fixes, re-tested 2026-07-24 after fixes)
...
| 36.3 | After returning from room creation, verify the Location assignment dropdown | The newly created room appears pre-selected in the correct `locationAssignments` row (the `controlName` `locationAssignments.{index}.locationId` was correctly set) | ✅ PASS — Newly created room "AR-Fix-Room" is pre-selected in the Location dropdown. The `autoForm()` fix (skipping async `load()` when draft exists) ensures the `updateDraftField` patched value is applied. |
| 36.4 | After returning from room creation, verify only the Serial Number field is marked dirty | Save/Cancel buttons appear (form is dirty), but only the Serial Number control has dirty state — not all form fields (the `dirtyKeys` array limits dirty marking to actually-modified controls) | ✅ PASS — Serial Number shows draft value "SN-AR-FIX-TEST". Form is dirty (`ng-dirty: true`). The draft was fully restored. The `beforePatch` callback correctly sized the `locationAssignments` FormArray before the draft was applied. |
