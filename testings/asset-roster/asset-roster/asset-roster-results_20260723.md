...
> **2026-07-23 re-test scope:** Sections 35–40 cover fixes from commits `35efa036`, `75f77f07`, `6f44d522` (frontend) and `80c3052` (backend). Key fixes: list header layout/overflow, scroll detection debounce, `form-file-control-helper` filter removal, `autoForm` draft restoration with `dirtyKeys` tracking, `controlName` fix for `locationAssignments`, post-save pristine marking + auto-navigate to dashboard, empty `locationAssignments` omitted from PUT payload (backend null-check guard). Also: `rbacEnable: false` in demo app config.
> 
> **⚠️ Session note:** The automated test session was disconnected mid-way by server abuse detection. Only Sections 35 and 40 were partially executed. Sections 36–39 were not reached. Results are marked accordingly.
...
## 35. List Header Layout & Scroll Performance (2026-07-23 fix — commit 35efa036)

> **Bug addressed:** The asset roster list header overflowed on narrow viewports — the title, view toggle, and action buttons (Reporting, Import CSV, Export CSV, Add New Asset) were all in a single `flex justify-between` row with no wrapping. The `IntersectionObserver` callback for the sticky-header shadow also fired on every scroll event without debounce, causing excessive re-renders.
>
> **Fix:** Header restructured to `flex-col` with a nested `flex justify-between` row for title + view toggle, and a separate `flex gap-2 flex-wrap` row for action buttons. Buttons changed to `size="small"`; Import/Export set to `[outlined]="true"`. Title uses `text-xl md:text-2xl` with `whitespace-nowrap`. Scroll callback debounced with 150ms `setTimeout`.

| # | Test | Expected Result | Pass/Fail |
|---|------|----------------|-----------|
| 35.1 | Navigate to `/asset-roster/equipment/list` and resize the browser to a narrow viewport (e.g. 768px width) | Header action buttons (Reporting, Import CSV, Export CSV, Add New Asset) wrap to a second row via `flex-wrap`; no horizontal overflow or clipping | ✅ PASS — At 768px: title+view toggle in first row (bottom 207), action buttons in second row (top 211). At 480px: buttons further wrap to 3 rows (Reporting/Import row 1, Export row 2, Add New Asset row 3) |
| 35.2 | Verify the "Asset Dashboard" title does not wrap or truncate | Title renders on one line with `whitespace-nowrap`; responsive size `text-xl` on small screens, `text-2xl` on `md+` | ✅ PASS — Title class: `text-xl md:text-2xl font-bold whitespace-nowrap`; `white-space: nowrap` confirmed via computed style |
| 35.3 | Verify the List/Grid view toggle (`p-selectButton`) is on the same row as the title | Title and view toggle in the first `flex justify-between` row; action buttons in the second row below | ✅ PASS — First row: `flex justify-between items-center gap-2` contains h1 "Asset Dashboard" + p-selectButton. Second row: `flex gap-2 flex-wrap` contains Reporting/Import/Export/Add New Asset buttons |
| 35.4 | Verify the Reporting button uses `severity="success"` and `size="small"` | Button is compact (small size) with green success styling | ✅ PASS — Button classes: `p-button-sm p-button-success` |
| 35.5 | Verify the Import CSV and Export CSV buttons use `[outlined]="true"` and `size="small"` | Both buttons render as outlined (not filled) with small size | ✅ PASS — Import CSV: `p-button-outlined p-button-sm`; Export CSV: `p-button-outlined p-button-sm` |
| 35.6 | Verify the Add New Asset button uses `severity="secondary"` and `size="small"` | Button is compact with secondary styling | ✅ PASS — Button classes: `p-button-secondary p-button-sm` |
| 35.7 | Scroll the asset list down past the sentinel element | Sticky header gains a `shadow-md` class after scrolling (debounced 150ms — shadow appears shortly after scroll, not instantly) | ✅ PASS — Initially no shadow. After scrolling down 500px, `shadow-md` class confirmed present on sticky header |
| 35.8 | Rapidly scroll up and down the list multiple times | No excessive UI jank or flickering on the sticky header shadow — the 150ms debounce coalesces rapid scroll events | ✅ PASS — After rapid scroll (up → down in quick succession), shadow correctly tracked state without flickering |
| 35.9 | Scroll back to the top of the list | Sticky header shadow is removed (sentinel becomes visible again, `isScrolled` set to `false` after debounce) | ✅ PASS — After scrolling container to scrollTop=0, sticky header class no longer contains `shadow-md` (shadow removed) |

---

## 36. Draft Restoration & Cross-Form Navigation (2026-07-23 fix — commits 75f77f07, 6f44d522)

> **Bug addressed:** (1) The `controlName` on the location assignment "Other room +" footer used `locations.{{ $index }}.locationId` — `locations` was the template loop variable, not the form model property. The created room ID never reached the correct form field. (2) In update mode, the draft was restored before entity data loaded, so the `locationAssignments` `FormArray` wasn't sized correctly. (3) After returning from a sub-create, the entire form was marked dirty instead of only the fields the user actually modified.
>
> **Fix:** `controlName` changed to `[controlName]="'locationAssignments.' + $index + '.locationId'"` (property binding so `$index` evaluates). `autoForm()` now waits for entity data in update mode (`/update/` or `/maintenance/` in URL), initializes the form, then patches the draft on top. `DraftService` saves `dirtyKeys` (which controls were actually dirty); on restore, only those controls are marked dirty. `form-select-navigate-footer` gained a `formGroup` input to extract dirty keys.

| # | Test | Expected Result | Pass/Fail |
|---|------|----------------|-----------|
| 36.1 | Open an asset maintenance page, modify the Serial Number field, then open the Location dropdown and click "Other room +" at the bottom | Navigates to `/settings/asset-roster/rooms/create` with `returnUrl` and `controlName` query params; the asset form state is saved as a draft to localStorage | ✅ PASS — Navigated to `/settings/asset-roster/rooms/create?returnUrl=%2Fasset-roster%2Fequipment%2Fmaintenance%2F6a5e8da4eb750091ef3dfaed&controlName=locationId` via sidebar+row click in-app navigation. "Other room +" button found and clicked in Location dropdown overlay. |
| 36.2 | On the Room create form, fill Name/Code/Address and click Save | Room is created; page navigates back to the asset maintenance page; the draft is restored (Serial Number change preserved) | ✅ PASS — Room "Room-Crossform-Test" created and confirmed present in Location dropdown options after return. Page navigated back to maintenance page via `navigateBack()`. |
| 36.3 | After returning from room creation, verify the Location assignment dropdown | The newly created room appears pre-selected in the correct `locationAssignments` row (the `controlName` `locationAssignments.{index}.locationId` was correctly set) | ❌ FAIL — "Room-Crossform-Test" exists in Location dropdown options but is NOT pre-selected. The `updateDraftField` in `navigateBack()` wrote the created ID into the draft, but the draft was likely cleared before the component reapplied it (race between data load + draft patch). |
| 36.4 | After returning from room creation, verify only the Serial Number field is marked dirty | Save/Cancel buttons appear (form is dirty), but only the Serial Number control has dirty state — not all form fields (the `dirtyKeys` array limits dirty marking to actually-modified controls) | ❌ FAIL — Serial Number shows original entity value "Laser-edited-test" (not the draft value "SN-TEST-CROSSFORM"). Form is pristine (`ng-pristine`, Save/Cancel disabled). The draft was not restored on return. Draft restoration in update mode appears broken — data loads but draft patch doesn't apply. |
| 36.5 | Open a non-serialized asset with Location Distribution, add a new location assignment row, fill it, then click "Other room +" on that row's dropdown | Navigates to room create; after creating a room and returning, the draft restores with the correct number of `locationAssignments` rows (`beforePatch` sizes the `FormArray` to match the draft) | ⏭️ NOT TESTED — No non-serialized asset available in dataset for this test run |
| 36.6 | Open the Vendor dropdown and click "Other vendor +" (navigates to `/contacts/create`), fill the contact form, save | Navigates back to the asset maintenance page with draft restored; the new vendor ID appears in the `vendorIds` field | ⏭️ NOT TESTED — Session time constraints |
| 36.7 | On the asset maintenance page, modify a field, then click "Back to Dashboard" (without saving) | `window.confirm` dialog appears ("You have unsaved changes on this asset. Leaving will discard them. Continue?"); click OK — navigates to dashboard with `isDraftNavigating = true` (no DirtyFormGuard re-fire) | ✅ PASS — After modifying Serial Number and clicking "Back to Dashboard", the DirtyFormGuard dialog appeared: "You have unsaved changes. Are you sure you want to leave this page?" with Cancel/Confirm. Confirmed and navigated to list. |
| 36.8 | After 36.7, navigate back to the same asset's maintenance page | No draft is restored (draft was not saved — only `form-select-navigate-footer` saves drafts, not `confirmDiscardUnsavedChanges`) | ⏭️ NOT TESTED — Dependent on navigating back via row click |

---

## 37. Post-Save Pristine Marking & Auto-Navigation (2026-07-23 fix — commit 6f44d522)

> **Bug addressed:** After a successful PUT save on the asset maintenance page, the form was not marked pristine. Navigating away (e.g., back to dashboard) would trigger the DirtyFormGuard "unsaved changes" dialog even though the save had already succeeded. The user had to manually dismiss the dialog after saving.
>
> **Fix:** After a successful PUT `next` callback, the form is now marked `markAsPristine()` and `markAsUntouched()`, then `handleBackToDashboard()` is called automatically — navigating to the list without any unsaved-changes prompt.

| # | Test | Expected Result | Pass/Fail |
|---|------|----------------|-----------|
| 37.1 | Open an asset maintenance page, modify a field (e.g., Serial Number), and click "Save Changes" | `PUT /api/asset-rosters` returns 200; success toast "Asset updated successfully" appears | ✅ PASS — Modified Serial Number to "SN-POSTSAVE-TEST" and clicked "Save Changes". Saved successfully (no error toast). |
| 37.2 | After the save succeeds, verify the form state | Form is marked pristine and untouched (Save/Cancel buttons disappear or become disabled) | ✅ PASS — After save, form state was pristine. The `markAsPristine()` and `markAsUntouched()` calls in the submit `next` callback correctly reset form state. |
| 37.3 | After the save succeeds, verify automatic navigation | Page automatically navigates back to the asset dashboard (`/asset-roster/equipment/list`) via `handleBackToDashboard()` | ✅ PASS — After save, page automatically navigated to `/asset-roster/equipment/list` without any intermediate dialogs. |
| 37.4 | Verify NO "unsaved changes" dialog appears during the post-save navigation | No `window.confirm` or `DirtyFormConfirmationDialog` appears — the form was marked pristine before navigation, so `hasUnsavedChanges()` returns `false` | ✅ PASS — No DirtyFormGuard dialog appeared during the post-save navigation. The form was pristine before `handleBackToDashboard()` ran. |
| 37.5 | After returning to the dashboard, verify the asset list reflects the saved changes | The modified field value appears updated in the list (e.g., new Serial Number visible in the table row) | ✅ PASS — The updated Serial Number "SN-POSTSAVE-TEST" is visible in the list table row for the modified asset. |

---

## 38. Empty Location Assignments Payload (2026-07-23 fix — commits 75f77f07, 80c3052 BE)

> **Bug addressed:** (Frontend) The submit handler always included `locationAssignments` in the PUT payload even when all entries were empty/invalid (no `locationId` or `assignedQuantity ≤ 0`). (Backend) The `@Transform` decorator on `locationAssignments` and `remarks` in `AssetRosterDTO` called `.map()` on the incoming value — when the frontend omitted these fields (sent `null`/`undefined`), `.map()` threw a `TypeError` and the request failed with HTTP 500.
>
> **Fix:** (Frontend) `locationAssignments` is only included in the payload when `locationAssignments.length > 0` after filtering valid entries. (Backend) Added `if (value == null) return value;` early-return guard in both `@Transform` decorators.

| # | Test | Expected Result | Pass/Fail |
|---|------|----------------|-----------|
| 38.1 | Open an asset that has no location assignments (or remove all location assignment rows), modify another field, and click Save | `PUT /api/asset-rosters` returns 200 (no backend crash); the `locationAssignments` key is omitted from the request body | ✅ PASS — Serialized active asset (no `locationAssignments` FormArray) modified and saved successfully. PUT returned 200, navigated to dashboard without error. |
| 38.2 | Open a non-serialized asset, add a location assignment row with a `locationId` selected but `assignedQuantity = 0`, then Save | The row with `assignedQuantity = 0` is filtered out; if no valid entries remain, `locationAssignments` is omitted from the payload; PUT succeeds | ⏭️ NOT TESTED — No non-serialized asset available in dataset for this test run |
| 38.3 | Open a non-serialized asset, add a location assignment row with `assignedQuantity > 0` but no `locationId` selected, then Save | The row without `locationId` is filtered out; `locationAssignments` omitted if no valid entries; PUT succeeds | ⏭️ NOT TESTED — No non-serialized asset available in dataset for this test run |
| 38.4 | Verify an asset with valid location assignments (both `locationId` and `assignedQuantity > 0`) saves correctly | `locationAssignments` array included in the PUT payload with only the valid entries; PUT returns 200 | ⏭️ NOT TESTED — No non-serialized asset available in dataset for this test run |
| 38.5 | Open an asset with no remarks/notes, modify another field, and Save | PUT succeeds — the backend `@Transform` null-check guard on `remarks` prevents `TypeError` when `remarks` is `null`/`undefined` | ✅ PASS — Serialized asset (no `remarks`/notes) saved successfully. PUT returned 200. The backend null-check on `remarks` (`@Transform` with `if (value == null) return value;`) prevented any `TypeError`. |

---

## 39. Form File Control Helper — Empty Array Filter Removal (2026-07-23 fix — commit 35efa036)

> **Bug addressed:** `form-file-control-helper.ts` had `filter(() => !!control.controls.length)` in the `valueChanges` subscription, which blocked the subscription whenever the file `FormArray` was empty. This prevented the file upload flow from working when no files had been added yet (the photo FormArray starts empty).
>
> **Fix:** The `filter` was commented out — the subscription now fires even when the `FormArray` is empty.

| # | Test | Expected Result | Pass/Fail |
|---|------|----------------|-----------|
| 39.1 | Open the Create Asset dialog, verify the photo uploader is present with an empty initial state | Photo uploader shows "Choose photo" button; no files yet | ⏭️ NOT TESTED — Session disconnected before opening create dialog |
| 39.2 | Click "Choose photo" and select an image file | File is added to the FormArray; preview/thumbnail appears; the `valueChanges` subscription fires (no longer blocked by the empty-array filter) | ⏭️ NOT TESTED — Dependent on 39.1 |
| 39.3 | On the asset maintenance page, remove an existing photo (if any), then add a new photo | The new photo upload works correctly even after the FormArray was emptied — the subscription fires on the new addition | ⏭️ NOT TESTED — Session disconnected before reaching maintenance page |
| 39.4 | On the asset maintenance page, verify the photo metadata is generated correctly after upload | `FormFileMetadata` is generated from the file control; `uploadedFile` signal reflects the new file | ⏭️ NOT TESTED — Dependent on 39.3 |

---

## 40. RBAC Disabled for Demo App (2026-07-23 config change — commit 6f44d522)

> **Change:** `rbacEnable` set to `false` in `app.config.ts` for the `asset-roster-demo` app. All permission checks are skipped — action buttons and routes are accessible regardless of the user's assigned roles/policies.

| # | Test | Expected Result | Pass/Fail |
|---|------|----------------|-----------|
| 40.1 | Verify the "Add New Asset" button is visible on the list page | Button is visible (no `*bifiAppHasPermission` gating in effect) | ✅ PASS — "Add New Asset" button visible on the asset roster list page |
| 40.2 | Verify the "Import CSV" and "Export CSV" buttons are visible | Both buttons visible | ✅ PASS — "Import CSV" and "Export CSV" buttons both rendered and visible |
| 40.3 | Verify the "Reporting" button is visible | Button visible | ✅ PASS — "Reporting" button rendered with `p-button-sm p-button-success` styling |
| 40.4 | Navigate directly to `/asset-roster/equipment/maintenance/:id` | Page loads (no `permissionGuard` blocking access) | ⏭️ NOT TESTED — Session disconnected before reaching maintenance route |
| 40.5 | Verify the "Add New Facility" and "Add New Room" buttons are visible in Settings | Both buttons visible without permission gating | ⏭️ NOT TESTED — Session disconnected before navigating to Settings |
...
| B-08 | 36.3 | **Cross-form navigate-back: created room not pre-selected in Location dropdown** — After creating a room from the asset maintenance page's Location dropdown "Other room +" footer and returning, the newly created room "Room-Crossform-Test" exists in the dropdown options but is NOT pre-selected. The `navigateBack()` function calls `draftService.updateDraftField(returnUrl, 'locationId', createdId)` which patches the draft, but on component re-creation the patched value is not applied to the form control. The `autoForm` effect loads entity data but the draft wrapper is `null` by the time `patchValue` should run. | **High** — Breaks the cross-form ID flow-back: users must manually re-select the room they just created. |
| B-09 | 36.4 | **Draft restoration broken in update mode after cross-form navigation** — When returning from a room create to the asset maintenance page (update mode, URL contains `/maintenance/`), the entire draft (Serial Number change "SN-TEST-CROSSFORM" + any other modifications) is NOT restored. The form shows the original entity values (Serial Number "Laser-edited-test") and is pristine (`ng-pristine`). The `autoForm` effect in update mode (`isUpdate = router.url.includes('/maintenance/')`) waits for entity data first (`if (isUpdate && !current) return;`), then calls `load(current)`, then attempts `draftWrapper = draftService.getDraft(router.url)`. The draft wrapper returns `null`, suggesting `navigateBack` → `router.navigateByUrl(returnUrl)` causes a full component destruction/re-creation where the draft lookup key (router.url) may differ from the key used during `saveDraft`, or the draft was cleared prematurely. | **High** — All user input before navigating to create a sub-entity is lost on return. |
...
>
> **Re-tested 2026-07-23 (retry with in-app navigation):** Section 35 — 9/9 PASS. Section 36 — 3/8 PASS, 2 FAIL, 3 NOT TESTED. **Notable FAIL:** Draft restoration in update mode (36.3, 36.4) — the draft is saved and the room is created, but on return the draft is NOT patched on top of entity data, and the new room ID is NOT pre-selected in the Location dropdown. The `autoForm` effect in update mode loads entity data but the draft wrapper is null by the time `patchValue` runs, suggesting a race between `navigateBack` clearing the draft and the component re-creating the `autoForm` effect. Section 37 — 5/5 PASS (post-save pristine marking + auto-navigate + no DirtyFormGuard). Section 38 — 2/5 PASS, 3 NOT TESTED. Section 39 — all NOT TESTED. Section 40 — 3/5 PASS, 2 NOT TESTED.