...
Tested: 2026-07-27 (re-test AC-04 expand/collapse)
...
| 2.3 | Verify the dialog subtitle | Shows "Perform inspection for: {productModel} - {serialNumber}" | ✅ PASS — shows "Perform inspection for:" (typo "Peform" fixed to "Perform" in catalog. AC-03 verified fixed 2026-07-27.) |
...
| 14.4 | Click on a commissioning activity history entry | Entry expands or shows details | ✅ PASS — expand/collapse toggle working correctly. **Default (collapsed)**: shows tag badge, dates, cost, loggedBy. Performed/details text hidden. **After click**: chevron icon toggles, performed date and details text appear. **Click again**: collapses back. Verified: `expandedIds` signal + `@if` guard in `activity-history-section.html`. |
...
| # | Test | Expected Result | Pass/Fail |
|---|------|----------------|-----------|
| 15.1 | On an asset with NO commission record (status `awaiting-commissioning`) | Maintenance Service section is hidden or disabled | ✅ PASS — section HIDDEN (AC-06 fix: wrapped in `@if (assetRoster()?.status !== 'awaiting-commissioning' && assetRoster()?.status !== 'decommissioned')`). Verified: "Initiate Service" and PM buttons not present in DOM. |
...
|---|------|-------------|----------|
| B-01 | ~ | **No whitespace validation (AC-01)** — ✅ **RESOLVED 2026-07-22**: Replaced `Validators.required` with `NonWhitespaceValidators.nonWhitespaceRequired`. | **Fixed** |
| B-02 | ~ | **Decommissioned assets can be re-commissioned (AC-02)** — ✅ **RESOLVED 2026-07-22**. | **Fixed** |
| B-03 | 2.3 | **Commission dialog subtitle typo (AC-03)** — ✅ **RESOLVED 2026-07-27**: Fixed "Peform" → "Perform" in translation catalog. Verified dialog shows "Perform inspection for:" correctly. File: `asset-roster-translations.json:2274`. | **Fixed** |
| B-04 | 14.4 | **Activity History entries have no expand/collapse (AC-04)** — ✅ **RESOLVED 2026-07-27**: Added `expandedIds` signal + `toggleExpand()` method + chevron toggle button. Details hidden by default, expand on click. Verified: collapse default → performed/details hidden, click → shown, click again → hidden. Files: `activity-history-section.ts:40-53`, `activity-history-section.html:26-34,90-105`. | **Fixed** |
| B-05 | 14.5/14.6 | **Labeling inconsistency** — Activity History action button says "Add Attachment" but the dialog header says "Add File to commissioning from asset: null". Inconsistent terminology. | **Fixed 2026-07-22** |
| B-06 | 15.1 | **Maintenance section visible (not hidden) on awaiting-commissioning assets (AC-06)** — ✅ **RESOLVED 2026-07-27**: Wrapped `<bifi-app-maintenance-service-section>` in `@if` guard checking `assetRoster()?.status !== 'awaiting-commissioning' && assetRoster()?.status !== 'decommissioned'`. Verified: section hidden on awaiting-commissioning, visible on active. File: `asset-roster-edit-form.html:120-126`. | **Fixed** |
| B-07 | ~ | **Double toasts per action (AC-07)** — ✅ **RESOLVED 2026-07-22**. | **Fixed** |
| B-08 | ~ | **Decommission button shows on decommissioned assets (AC-08)** — ✅ **RESOLVED 2026-07-22**. | **Fixed** |
...
| ✅ PASS | 60 |
| ❌ FAIL | 0 |
| ⚠️ PARTIAL / BUG / NOTE | 2 |
| ⏭️ SKIPPED / N/A | 14 |

> **Re-tested 2026-07-27 for AC-03, AC-04, AC-06:** Commission dialog subtitle typo fixed (2.3 ✅). Activity History expand/collapse working (14.4 ✅). Maintenance section hidden on awaiting-commissioning (15.1 ✅). B-03, B-04, B-06 moved to Fixed. 60 PASS total (+3 from previous run).
...
- Tested on an existing active asset with maintenance activity history entries
- Expand/collapse tested: verified default collapsed state, expand on chevron click, collapse on second click
