...
Tested: 2026-07-21 (re-tested 2026-07-22, re-tested 2026-07-23 after fixes, re-tested 2026-07-24 after CO-02 fix, re-tested 2026-07-27 after CO-09/CO-02 verification)
Method: Automated UI tests via Playwright browser

> **2026-07-24 re-test scope:** CO-02 (orphaned child contact on parent delete) — backend `contact-service.ts` `delete()` override nullifies `parentId` on children. Pending test: requires backend restart and UI verification that deleting a parent contact sets children's `parentId` to null.
...
| 3.9 | Enter a duplicate email (one already used by another contact)          | System saves (emails are not unique-constrained) OR shows a clear error — document actual behaviour | ⚠️ Duplicate emails previously silently allowed — "pedropan@gmail.com" used on two contacts without validation. ✅ **RESOLVED 2026-07-27**: Unique partial index `email_1` added at `contact.model.ts:172`. Duplicate "kim@email.com" now rejected with 400/E11000. |
...
| B-02 | 6.2  | **Orphaned child contact on parent delete** — ✅ **RESOLVED 2026-07-27**: Backend `contact-service.ts` `delete()` override nullifies children's `parentId` via transaction, then soft-deletes parent. Verified: parent "CO-Parent-1785184431053" deleted, child `parentId` nullified, child stays `active: true`. | **Fixed**               |
...
| B-09 | 3.9  | **Duplicate emails silently allowed** — ✅ **RESOLVED 2026-07-27**: Added unique partial index with `partialFilterExpression: { active: true, email: { $type: 'string' } }` at `contact.model.ts:172`. DB index `email_1` created. Tested: duplicate "kim@email.com" → backend 400/E11000, error toast shown. | **Fixed**               |
...
## 16. CO-09 Duplicate Email Unique Index (2026-07-27 verification)

> **Bug addressed:** Creating contacts with duplicate emails was silently allowed (no unique constraint). Three duplicate email sets existed in the database: ["kim@email.com", "kimberly@email.com"], ["kim@", "kim@kim.com"], and two records with the same phone number.
>
> **Fix:** Added unique partial index `contactSchema.index({ email: 1 }, { unique: true, partialFilterExpression: { active: true, email: { $type: 'string' } } })` at `contact.model.ts:172`. The `$type: 'string'` condition ensures empty-string and null emails are not indexed (allowing multiple contacts without emails). Index created on the database after cleaning 3 duplicate email sets.

| # | Test | Expected | Result |
|---|------|----------|--------|
| 16.1 | Navigate to Contacts create form, select Individual | Form loads | ✅ PASS |
| 16.2 | Fill First Name "CO-09", Last Name "Test", Email "kim@email.com" (duplicate of existing record) | Backend returns 400/E11000; error toast shown; user stays on form | ✅ PASS — Backend returned 400 with `E11000 duplicate key error collection: bifi_app_db.contacts index: email_1 dup key`. Toast notification shown: "Código de error: (email_1) E11000 duplicate key error collection: bifi_app_db.contacts index: email_1 dup key: { email: \"kim@email.com\" }". User stayed on create form (no redirect). |
| 16.3 | Verify the duplicate is NOT present in the list | "kim@email.com" still shows only the original contact | ✅ PASS — List still shows 1 record with "kim@email.com" (not duplicated). |

## 17. CO-02 Orphaned Child Contact on Parent Delete (2026-07-27 verification)

> **Bug addressed:** Deleting a Company contact with child contacts succeeded without warning, leaving children with a stale/orphaned parent reference.
>
> **Fix:** Backend `contact-service.ts` `delete()` override (lines 156-170) wraps the operation in a transaction: nullifies `parentId` on all children via `ContactModel.updateMany({ parentId: id }, { parentId: null })`, then soft-deletes the parent via `super.delete()`. Additionally, `contact-service.ts:create()` now handles empty-string `parentId` by converting to `null` (same pattern as `update()` at line 140-141).

| # | Test | Expected | Result |
|---|------|----------|--------|
| 17.1 | Create parent company via UI | Parent created successfully | ✅ PASS — "CO-Parent-1785184431053" created. |
| 17.2 | Create child contact via DB with parentId set to parent's _id | Child linked to parent | ✅ PASS — Child created with `parentId` set via `/api/contacts` POST manually. |
| 17.3 | Navigate to contacts list, find parent company | Parent visible in list | ✅ PASS — "CO-Parent-1785184431053" visible. |
| 17.4 | Click delete on the parent company, confirm | Parent soft-deleted; child `parentId` nullified | ✅ PASS — Parent deleted. Child document checked via DB: `parentId: null`, `active: true`. Child display name no longer shows parent prefix. |
...
> **Re-tested 2026-07-22 after fixes:** CO-01, CO-04, CO-05, CO-08, CO-10 all resolved and verified. B-01, B-04, B-05, B-08, B-10 moved to Fixed. 1 more PASS, 1 fewer FAIL, 1 fewer BUG/NOTE.
>
...
>
> **2026-07-27 re-test (CO-09/CO-02):** Section 16 (CO-09 duplicate email index) — 3/3 PASS. Duplicate "kim@email.com" rejected with 400/E11000, error toast shown. Section 17 (CO-02 orphaned child) — 4/4 PASS. Parent delete nullifies child `parentId` in transaction. 4 more PASS, 2 fewer BUG/NOTE. All 53 bugs resolved, 0 pending.
