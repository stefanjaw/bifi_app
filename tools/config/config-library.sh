...
for LIB in $LIBS; do
    ...
    if [ -d "$DIST_DIR" ]; then
        ...
        if [ -n "$TGZ_FILE" ]; then
            ...
            echo "Warning: No .tgz file found in $DIST_DIR for library $LIB"
        fi
    else
        echo "Warning: Distribution directory $DIST_DIR does not exist for library $LIB"
    fi
done
...
for LIB in $LIBS; do
    ...
    if [ -f "$PROVIDER_FILE" ]; then
        ...
        if [ -n "$PROVIDER_NAME" ]; then
            ...
            echo "Warning: No provider found in $PROVIDER_FILE"
        fi
    else
        echo "Warning: Provider file does not exist: $PROVIDER_FILE"
    fi
done
